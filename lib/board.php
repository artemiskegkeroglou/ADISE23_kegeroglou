...
function add_valid_moves_to_piece(&$board,$b,$x,$y) {
	...
	if($board[$x][$y]['piece_color']==$b) {
		switch($board[$x][$y]['piece']){
			case 'K1': $number_of_moves+=k1_moves($board,$b,$x,$y);break;
			case 'K2': $number_of_moves+=k2_moves($board,$b,$x,$y);break;
		}
	} 
	return($number_of_moves);
}
...
function k1_moves(&$board,$b,$x,$y) {
	return(0);
}
function k2_moves(&$board,$b,$x,$y) {
	/*$directions = [
		[1,0],
		[-1,0],
		[0,1],
